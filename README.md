...
- **A queue you can actually edit** — drag to reorder anything ahead of you, move songs between **Up Next** and **Coming Up**, and **X out** any song you don't want to hear — from the side panel or from the fullscreen visualizer's queue drawer. It leaves the queue; the file is never touched.
...
- **Optional in-app downloader** — paste `Artist - Track` (or a URL) and pull new songs straight into your library, preferring the original/explicit master. Wraps `yt-dlp`, previews each match, and lets you swap the exact version before downloading. New grabs land in a **Downloaded** playlist, and the downloader **docks to the side** so it keeps running in the background while you keep browsing. The *only* networked feature — opt-in; see [`DOWNLOADER.md`](DOWNLOADER.md).
- **It's hiding more than music** — poke around and you'll turn up a little dock of tools… and an **arcade** that floats over the app while your library keeps playing, with a spectrum glowing under the game in fullscreen. Half the fun is finding them. _(Spoilers in [`TOOLS.md`](TOOLS.md), if you really must.)_
